...
void TAVLCom::PonerCaminoDer(TListaCom &listanodos) {
    if(listanodos.EsVacia()) {
        listanodos.InsCabeza(this->Raiz());
    } 
    else {
        listanodos.InsertarD(this->Raiz(), listanodos.Ultima());
    }
    ...
    if(!this->raiz->de.EsVacio()) {
        this->raiz->de.PonerCaminoDer(listanodos);
    } else {
        this->raiz->iz.PonerCaminoDer(listanodos);
    }
}

void TAVLCom::PonerCaminoIzq(TListaCom &listanodos) {
    if(listanodos.EsVacia()) {
        listanodos.InsCabeza(this->Raiz());
    }
    else {
    ...
    if(!this->raiz->iz.EsVacio()) {
        this->raiz->iz.PonerCaminoIzq(listanodos);
    } else {
        this->raiz->de.PonerCaminoIzq(listanodos);
    }
}

void TAVLCom::Caminos_AVLaux(TComplejo buscando, TListaCom &listanodos) {
    if(this->Raiz() == buscando) {
        if((int)buscando.Re()%2 == 0) {
            this->PonerCaminoDer(listanodos);
        } else {
            this->PonerCaminoIzq(listanodos);
        }
    } 
    else {
        if(buscando > this->Raiz()) {
            this->raiz->de.Caminos_AVLaux(buscando, listanodos);
        } else {
            this->raiz->iz.Caminos_AVLaux(buscando, listanodos);
        }
    }
}


TListaCom TAVLCom::Caminos_AVL(TVectorCom &vectorcom) {
    TListaCom listanodos;
    TComplejo buscando;
    int vaciosFinal = 0; 
    if(vectorcom.Tamano() == 0) {
        return listanodos;
    ...
    for(int i=0; i<vectorcom.Tamano(); i++) {
        buscando = vectorcom[i];
        if(!this->Buscar(buscando)) {
            TComplejo vacio;
            if(listanodos.EsVacia()) {
                listanodos.InsCabeza(vacio);
            } else {
                listanodos.InsertarD(vacio, listanodos.Ultima());
            }
        }
        else if(buscando == this->Raiz()) {
            if((int)buscando.Re()%2 == 0) {
                this->PonerCaminoDer(listanodos);
            } else {
                this->PonerCaminoIzq(listanodos);
