...
//------------------------------------------------------------------------
void TABBCom::ImprimirNivel(int nivel) {
    if(nivel == 0) {
        cout << ' ' << this->Raiz();
        return;
    }
    ...
    if(!this->nodo->iz.EsVacio()) {
        this->nodo->iz.ImprimirNivel(nivel-1);
    }

    if(!this->nodo->de.EsVacio()) {
        this->nodo->de.ImprimirNivel(nivel-1);
    }

}

int TABBCom::ObtenerNivel(TComplejo complejo) {
    int nivel = 0;
    ...
    if(complejo > this->Raiz()) {
        return this->nodo->de.ObtenerNivel(complejo) + 1;
    } else {
        return this->nodo->iz.ObtenerNivel(complejo) + 1;
    }
    

}

TComplejo* TABBCom::MostrarNiveles(TListaCom &listacom) {
    //ELIMINACION DE LAS POSICIONES PARES DE LA LISTA--------------------------
    TComplejo* arrayDinamico;
    if(listacom.Longitud() == 0) {
        return arrayDinamico;
    }

    TListaPos* posicionesBorrar = new TListaPos[listacom.Longitud()];
    int contadorPos = 1;
    int contadorBorrar = 0;
    for(TListaPos posicion = listacom.Primera(); !posicion.EsVacia(); posicion = posicion.Siguiente()) {
        if(contadorPos%2 == 0) {
            posicionesBorrar[contadorBorrar] = posicion;
            contadorBorrar++;
        }
        contadorPos++;
    }

    for(int i=0; i<contadorBorrar; i++) {
        listacom.Borrar(posicionesBorrar[i]);
    }
    //FIN ELIMINACION DE LAS POSICIONES PARES DE LA LISTA---------------------

    //PASAR ELEMENTOS DE LA LISTA A ARRAY DINAMICO DE TCOMPLEJO---------------
    arrayDinamico = new TComplejo[listacom.Longitud()];
    int contadorPasoArray = 0;
    for(TListaPos posicion = listacom.Primera(); !posicion.EsVacia(); posicion = posicion.Siguiente()) {
        arrayDinamico[contadorPasoArray] = listacom.Obtener(posicion);
        contadorPasoArray++;
    }
    //FIN PASAR ELEMENTOS DE LA LISTA A ARRAY DINAMICO DE TCOMPLEJO-----------

    //ELIMINACION E INSERCION DE NUEVOS DATOS EN TABBCOM----------------------

    if(!this->EsVacio()) {
        this->~TABBCom();
    }

    for(int i=0; i<listacom.Longitud(); i++) {
        this->Insertar(arrayDinamico[i]);
    }

    //FIN ELIMINACION E INSERCION NUEVOS DATOS EN TABBCOM---------------------

    //IMPRIMIR POR PANTALLA---------------------------------------------------

    for(int i=0; i<listacom.Longitud(); i++) {
        cout << "{Elemento: " << arrayDinamico[i] << ". ";
        int nivel = this->ObtenerNivel(arrayDinamico[i]);
        cout << "Elementos de su nivel:";
        this->ImprimirNivel(nivel);
        cout << '}' << endl;
    }

    //FIN IMPRIMIR POR PANTALLA-----------------------------------------------
