...
int TAVLCom::BuscaAVLizq(TComplejo buscando) {
    if(this->Raiz() == buscando) {
        return 1;
    }

    if(buscando < this->Raiz()) {
        return this->raiz->iz.BuscaAVLizq(buscando);
    } else {
        return this->raiz->de.BuscaAVLder(buscando);
    }
}

int TAVLCom::BuscaAVLder(TComplejo buscando) {
    if(this->Raiz() == buscando) {
        return 2;
    }
...
int* TAVLCom::BuscaAVL(TListaCom &listacom) {
    TComplejo buscando;
    int posicionSalida = 0;
    int* buscados = new int[listacom.Longitud()];
    ...
    for(TListaPos posicion = listacom.Primera(); !posicion.EsVacia(); posicion = posicion.Siguiente()) {
        buscando = listacom.Obtener(posicion);

        if(!this->Buscar(buscando)) {
            buscados[posicionSalida] = 0;
            
        } 
        else if(buscando == this->Raiz()) {
            buscados[posicionSalida] = 3;
        }
        else {
            if(buscando > this->Raiz()) {
                buscados[posicionSalida] = this->raiz->de.BuscaAVLder(buscando);
            }
            else {
                buscados[posicionSalida] = this->raiz->iz.BuscaAVLizq(buscando);
            }
